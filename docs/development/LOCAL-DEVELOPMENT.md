...
This is the **Rails 8.1.3.1 / Ruby 4.0.6 local development environment**, run via Docker Compose. Production runs the same Rails/Ruby versions via Heroku buildpacks, not this Compose setup (see [PRODUCTION-DOCKER.md](PRODUCTION-DOCKER.md) for the separate, portable production image, and [../archive/UPGRADE-PLAN.md](../archive/UPGRADE-PLAN.md) for how the app got here — it passed through Ruby 3.0.7 and Rails 6.1.7.10 / 7.0.10 as intermediate migration steps before reaching this runtime; Rails 6.1.7.10 removed the unavailable `mimemagic 0.3.5` dependency from the original lockfile).
...
Bootsnap is updated and enabled. The image uses Bundler 4.0.16, Node 24 and Debian Trixie, with ffmpeg, ImageMagick and libvips installed explicitly. Ruby 4.0.6 and the updated dependency set pass 162 examples with zero failures and 29 inherited placeholders, plus autoloading and asset-compilation checks. Tests exercise authorization boundaries, required associations, Rails-default cookie, redirect and CSRF compatibility, signup-abuse prevention, account activation and single-use password recovery with non-delivering mail, real WAV and browser-style WebM/Opus conversion, libvips image resizing, accepted hardened image loaders, and rejection plus blocked decoding of a real EXR payload disguised as an allowed JPEG upload. The production image is documented in [PRODUCTION-DOCKER.md](PRODUCTION-DOCKER.md).
